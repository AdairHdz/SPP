...
namespace DataPersistenceLayer.Repositories
{
	public class PracticionerRepository : Repository<Practicioner>, IPracticionerRepository
	{
		...
		public string GetPracticionerProject(string enrollment)
		{
			Assignment assignment = _context.Set<Assignment>().SingleOrDefault(Assignment => Assignment.Enrollment == enrollment
			&& (Assignment.Status == AssignmentStatus.Assigned) && (Assignment.Project.Status == ProjectStatus.FILLED
			|| Assignment.Project.Status == ProjectStatus.ACTIVE));
			Project project = _context.Set<Project>().SingleOrDefault(Project => Project.IdProject == assignment.IdProject);
			return project.NameProject;
		}

		public IList<ActivityPracticioner> GetPracticionerPartialReports(string enrollment)
		{
				return _context.Set<ActivityPracticioner>().Include(Act => Act.Activity).Where(ActivityPracticioner => ActivityPracticioner.Enrollment == enrollment 
				&& ActivityPracticioner.Activity.ActivityType == ActivityType.PartialReport).ToList();
		}

		public IList<ActivityPracticioner> GetPracticionerMonthlyReports(string enrollment)
		{
				return _context.Set<ActivityPracticioner>().Include(Act => Act.Activity).Where(ActivityPracticioner => ActivityPracticioner.Enrollment == enrollment
